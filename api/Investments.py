...
def Investments():
    ...
    for i in range(number_of_markets):
        results = results + markets[i] + ":<br />\n<br />\n"
        if len(coins_in_market[i]) > 1:
            for j in range(len(coins_in_market[i])):
                results = results + coins_in_market[i][j] + ":<br />\nAmount: " + str("{0:.2f}".format(amounts[i][j])) + "<br />\nPrice: " + str("{0:.2f}".format(prices[i][j])) + "<br />\nCost: " + str("{0:.2f}".format(costs[i][j])) + "<br />\nCurrent Value: " + str("{0:.2f}".format(values[i][j])) + "<br />\nProfit/Loss: " + str("{0:.2f}".format(gain_loss[i][j])) +"<br />\n<br />\n"
        else:
            results = results + coins_in_market[i][0] + ":<br />\nAmount: " + str("{0:.2f}".format(amounts[i][0])) + "<br />\nPrice: " + str("{0:.2f}".format(prices[i][0])) + "<br />\nCost: " + str("{0:.2f}".format(costs[i][0])) + "<br />\nCurrent Value: " + str("{0:.2f}".format(values[i][0])) + "<br />\nProfit/Loss: " + str("{0:.2f}".format(gain_loss[i][0])) + "<br />\n<br />\n"
    for i in range(number_of_markets):
        results = results + "<br />\n" + markets[i] + ":<br />\n" + "total costs: " + str("{0:.2f}".format(total_costs[i])) + "<br />\n" + "total values: " + str("{0:.2f}".format(total_values[i])) + "<br />"
        results = results + "Profit/loss: " + str("{0:.2f}".format(100*(total_values[i] - total_costs[i])/(total_costs[i]))) + " %<br />\n"
    results = results + "<br />\noverall cost: " + str("{0:.2f}".format(overall_cost)) + "<br />\n" + "overall value: " + str("{0:.2f}".format(overall_value)) + "<br />\n"
    results = results + "Profit/loss: " + str("{0:.2f}".format(100*(overall_value - overall_cost)/(overall_cost))) + " %<br />\n"
    return results
